...
# （仅密钥）。本脚本按需开启 sshd,密码登录用一个排在它之前的 drop-in 覆盖(sshd 首个匹配生效)。
set -e
...
systemctl enable sshd >/dev/null 2>&1 || true  # 开机自启(live 无影响，保留无害)
# 关键：必须让运行中的 sshd 重载才会应用上面新写/删的 drop-in。原先用 `enable --now`,
# sshd 已在运行时它直接返回成功、根本不重载(`|| restart` 也就不触发)→ 改动不生效(root 登不上、
