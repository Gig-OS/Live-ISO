#!/bin/bash
# gigos-zfs-prebootloader.sh — 装机时在 Calamares 的 grubcfg/bootloader 模块之前执行
# (shellprocess@zfspre,目标 chroot 内,dontChroot:false → ROOT=/)。仅 ZFS 根触发。
#
# 为什么需要:ZFS 根安装时,Calamares 的 bootloader 模块仍会执行 `grub-install`,但 GRUB 读不了
# ZFS 池(尤其原生加密)→ grub-install 退 1 → bootloader 模块失败 → Calamares 致命中止，
# 装机后处理(shellprocess@zfs,装 ZFSBootMenu)根本轮不到执行(实测就卡在这：安装失败/启动加载器安装出错)。
# 解法：本步把 grub-install / grub-mkconfig 临时换成 no-op(原件挪到 *.gigos-real),让 grubcfg/bootloader
# 模块以成功状态空执行过；真正的引导器(ZFSBootMenu)由其后的 shellprocess@zfs 安装，并把这两个工具还原。
...
for t in grub-install grub-mkconfig; do
    ...
    if [ "$t" = grub-install ]; then
        # grub-install:no-op 不够。Calamares bootloader 模块执行完 grub-install 后会无条件
