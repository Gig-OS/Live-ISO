...
# 为社区 binhost 的签名公钥建立 portage 信任。
# binrepos.conf 设了 verify-signature = true，portage 用 /etc/portage/gnupg 这个独立钥匙环验签，
# 而非用户的个人钥匙环。该钥匙环由 getuto 初始化，公钥来自 sec-keys/openpgp-keys-gentoozh，
# 导入后还要 lsign 一次，否则密钥虽已存在但不被信任，验签仍不通过。
# 任一步失败只警告不中止：验签未配好时 portage 会拒绝该源的包并改为编译源码，属可用的降级。
...
if ! [ -f "${WORKDIR}/squashfs${GIGOS_KEY_ASC}" ]; then
    ...
    if crun "gpg --homedir /etc/portage/gnupg --import ${GIGOS_KEY_ASC}"; then
        # lsign 需要 getuto 存放在 /etc/portage/gnupg/pass 的口令。
